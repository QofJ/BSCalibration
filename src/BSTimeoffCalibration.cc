...
int BSTimeoffCalibration::Init()
{
    ...
    dir_set[0] = -10; dir_set[1] = -10; dir_set[2] = -10;
	core[0] = 0; core[1] = 0;
    a = 0; c0 = 0;
    Sigma = 0;
    Sigma_mean = 0;
    Nevents = 0;
    Dazcos[0] = 0; Dazcos[1] = 0; Dazcos[2] = 0;
...
int BSTimeoffCalibration::SetToffset(const char* file)
{
    ...
    {
        fscanf(f, "%s %d %lf\n", type, &id, &toff);
        if (strcmp(type, "ED") != 0)break;
        if (id < 0 || id >= MNED)continue;
        ToffsetED[id] = toff;
    }
    fclose(f);
    return 0;
...
int BSTimeoffCalibration::SetSource(double mjd, double ra, double dec, int flag)
{
	double zen, az;
	double Mjd;
	if (mjd < 0)Mjd = lhevent->GetMjd();
	else Mjd = mjd;
	precession2New(Mjd, &ra, &dec);
	e2h_argo(Mjd, ra, dec, &zen, &az);
	theta = zen * DD2R; phi = az * DD2R;
    if (flag == 0)
    {
        dir_set[0] = sin(theta) * cos(phi); dir_set[1] = sin(theta) * sin(phi); dir_set[2] = cos(theta);
        this->c0_set = this->c0;
    }
    else if (flag == 1)
    {
...
int BSTimeoffCalibration::SetBadHit(TClonesArray& tHits, int np, const char* style)
{
    ...
    if (strcmp(style, "ED") == 0) {
        for (i = 0; i < np; i++) {
            tHit = (LHHit*)((tHits)[i]);
            id = tHit->GetId();
            if (geom->GetEDId2(id) < 0)tHit->SetStatus(-2);
        }
    }
		...
		for (i = 0; i < np; i++) {
			tHit = (LHHit*)((tHits)[i]);
			id = tHit->GetId();
			if (geom->GetMDId2(id) < 0)tHit->SetStatus(-2);
		}
	}
	return 0;
}
...
int BSTimeoffCalibration::SetPreToffset(const char* meanCalfile, const char* Pre_Toffsetfile)
{
    ...
    {
        fscanf(f1, "ED %d %lf\n", &id, &time);
        if (Pre_ToffsetED[id] > -500)Pre_ToffsetED[id] += time;
    }
...
int BSTimeoffCalibration::GenerateToffset(const char* style, int MPmode, double delta, double step)
{
    ...
    if (strcmp(style, "ED") == 0)
    {
        if (MPmode == 0) {
            ...
            for (int i = 0; i < MNED; i++)
            {
                if (cal_statED[i] == 0)ToffsetED[i] = 0;
                else ToffsetED[i] += step * (conical_residualsED[i] / cal_statED[i]);
            }
        }
        else if (MPmode == 1)
        {
            for (int i = 0; i < MNED; i++)
            {
                if (cal_statED[i] == 0) continue;
                //else ToffsetED[i] = GetMPValue(i, delta);
                else ToffsetED[i] += step * (GetMPValue(i, delta) - ToffsetED[i]);
            }
        }
        else
        {
            for (int i = 0; i < MNED; i++)
            {
                if (cal_statED[i] == 0) continue;
                //else ToffsetED[i] = GetMPValue(i, delta);
                else ToffsetED[i] += step * GetMPValue(i, delta);
            }
            //ApplyDaz();
        }
    ...
    */
    if (maxid > 0 && geom->Getxyz(maxid, x, y, z, 1, style) > 0) {
        ...
        for (i = 0; i < np; i++) {
            tHit = (LHHit*)((tHits)[i]);
            if (tHit->GetStatus() > 0) {
                if (tHit->GetTime() - ToffsetED[tHit->GetId()] < maxt || tHit->GetTime() - ToffsetED[tHit->GetId()] >= maxt + twind)tHit->SetStatus(0);
                else {
...
int BSTimeoffCalibration::conicalfit(TClonesArray& tHits, int np, float alpha, const char* style, int IsSet)
{
    ...
    if (IsSet == 0)
    {
        dirl = dir[0]; dirm = dir[1]; dirn = dir[2];
    }
    else
    {
        dirl = dir_set[0]; dirm = dir_set[1]; dirn = dir_set[2];
    }
    ...
    for (int iter = 0; iter < 1000; iter++)
    {
        ...
        for (i = 0; i < np; i++)
        {
            ...
            if (iter > 0)
            {
                ...
                if (dt > 0 || dt < -20)w = w * exp(-0.5 * pow(dt / 10., 2));
...
int BSTimeoffCalibration::conicalfit_dir(TClonesArray& tHits, int np, float alpha, const char* style, int nmode, double mjd, double ra0, double dec0)
{
    ...
    if (nmode != 0) {
        ...
        the = the * DD2R; phi_temp = phi_temp * DD2R;
    ...
    if (Flag > 0.5 && dt <= 1. && dt >= 0.) {
        ...
        if (phi_temp < 0.) phi_temp += DPI * 2.;
        dir[0] = dirl;
        dir[1] = dirm;
        dir[2] = dirn;
        if (AA < 0.5)this->a = r[2];
        else this->a = alpha;
        this->c0 = r[3];
        this->theta = the;
        this->phi = phi_temp;
...
double BSTimeoffCalibration::GetRefTime(int IsSet)
{
    ...
    {
        dirl = dir[0]; dirm = dir[1]; dirn = dir[2];
        a_temp = this->a; c0_temp = this->c0;
    }
    else
    {
        dirl = dir_set[0]; dirm = dir_set[1]; dirn = dir_set[2];
        a_temp = this->a; c0_temp = this->c0_set;
    }
...
int BSTimeoffCalibration::eventcallineED_dir()
{
    ...
    if (!(NfiltE >= 3 && planarfit(*HitsE, ned, "ED") == 0))return -1;
 
    if (conicalfit_dir(*HitsE, ned, 0.035, "ED") != 0)return -1;
...
int BSTimeoffCalibration::ApplyDaz()
{
    ...
    {
        if (cal_statED[i] == 0)continue;
        if (geom->Getxyz(i, x, y, z, 1, "ED") < 0)continue;
...
int BSTimeoffCalibration::eventcallineED()
{
    ...
    if (conicalfit(*HitsE, ned, 0.035, "ED") != 0)return -1;
    NfiltE = noisefilter(*HitsE, ned, -50, 100, rWind + 100, "ED");
    if (NfiltE > 200)
    {
        noisefilter(*HitsE, ned, -50, 100, rWind + 300, "ED");
        if (conicalfit(*HitsE, ned, -0.035, "ED") != 0)return -2;
    }
    else
    {
        if (conicalfit(*HitsE, ned, 0.035, "ED") != 0)return -2;
