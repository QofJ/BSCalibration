...
int Calibration(BSTimeoffCalibration *BS, double rin, double rout, double El, double dCx = 0, double dCy = 0)
{
	...
	for (i = 2021202; i < 2022250; i++)
	{
		...
		for (j = 0; j < NTotalEntry; j++)
		{
			...
			if (drr > (rout + 3) * (rout + 3))
				continue;
			if (pow(10., E) < El)
				continue;
			if ((log10((num4 + 0.0001) / npe3) > -2.3))
				continue;
			tree->GetEntry(j);
			// printf("Ra = %lf, Dec = %lf\n", ra, dec);
...
int WritetxtFile(const char *outroot, const char *outnt, const char *outtxt = "/eos/user/q/qijincan/brightsource/result/calEDs_temp.txt")
{
	int i, ID, NED;
	double id, toff;
...
int main(int argc, char *argv[])
{
	...
	{
		strArgv1 = strArgv1.substr(0, pos);
	}
	strcat(ToffsetTxt, strArgv1.c_str());
	strcat(ToffsetTxt, ".txt");
