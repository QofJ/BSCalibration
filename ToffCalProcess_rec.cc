...
int Calibration(BSTimeoffCalibration* BS, double rin, double rout, double El, double dCx = 0, double dCy = 0)
{
	...
	for (i = 2021202; i < 2022250; i++)
	{
		...
		for (j = 0; j < NTotalEntry; j++)
		{
			...
			//printf("c0 = %lf\n", c0);
			//BS->SetSource(mjd, Source_Ra + dCx, Source_Dec + dCy,1);
			//printf("c0' = %lf\n", BS->GetC0());
...
int main(int argc, char* argv[])
{
	...
	for (i = 0; i < ITER; i++)
	{
		bscal->SetITER(i);
		bscal->Init();
		//if(i ==0)bscal->SetToffset("/home/lhaaso/qijincan/KM2ADatarec_V2/config/TrueCal_mean_211001_221001_norm.txt");
		Calibration(bscal, rin, rout, El, dCenterx, dCentery);
		bscal->GenerateToffset("ED", 2);
		//else bscal->GenerateToffset("ED", 2, 10, 0.5);
