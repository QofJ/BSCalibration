...
int Calibration(BSTimeoffCalibration* BS, double rin, double rout, double El, double dCx = 0, double dCy = 0)
{
	...
	char filehead[200] = "root://eos01.ihep.ac.cn//eos/user/q/qijincan/brightsource/result/LHE_BK";
	char file[200];
	char suffix[20] = ".root";
	const char* filedate;
...
int main(int argc, char* argv[])
{
	if (argc != 8)
	{
		printf("For Experiment Data:\n");
		printf("Usage: %s <outfile> <ITER> <rin> <rout> <El> <dCenterx> <dCentery>\n", argv[0]);
		return 0;
	}
	int i, j, ITER = 20;
	double* count, * toff;
	...
	rout = atof(argv[4]);
	El = atof(argv[5]);
	dCenterx = atof(argv[6])-Source_Ra;
	...
	for (i = 0; i < ITER; i++)
	{
		...
		bscal->Init();
		if(i==0){
			bscal->SetPreToffset("/home/lhaaso/qijincan/KM2ADatarec_V2/config/TrueCal_mean_211001_221001.txt","/home/lhaaso/qijincan/KM2ADatarec_V2/config/pre_TimeOffset_Gaus.txt");
			...
			bscal->SetToffset("/home/lhaaso/qijincan/KM2ADatarec_V2/config/pre_TimeOffset_Gaus.txt");
		}		
