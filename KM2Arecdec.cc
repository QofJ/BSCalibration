...
int main(int argc, char* argv[])
{
	...
	char filepath[200];
	char decodefile[200];
	char outhead[100] = "root://eos01.ihep.ac.cn//eos/user/q/qijincan/brightsource/result/LHE";
	char outpath[200];
	char stemp[200];
	...
	while (!feof(filelist))
	{
		...
		for (i = 0; i < NTotalEntry; ++i)
		{
			...
			h2e_argo(MJD, Theta * DR2D, Phi * DR2D, &Ra, &Dec);
			precession2Old(MJD, &Ra, &Dec);
			if ((Ra < 75 || Ra > 90 || Dec < 15 || Dec > 30))continue;

			E = recer50new3(age, size, Theta);

			if ((log10((NuM4 + 0.0001) / NpE3) < -2.) && (E != -1))
			{
